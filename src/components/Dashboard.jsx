...
// Fix bug icon marker Leaflet default agar muncul aman di React/Vite
...
export default function Dashboard() {
  ...
  // 🌟 STATE STRATEGUR VIEW MODAL (Peta vs Tabel)
  const [activeView, setActiveView] = useState('tabel'); // Default ke 'tabel' sesuai screenshot kamu

  // 🌟 STATE UTAMA UNTUK FILTER HORIZONTAL
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedSeverity, setSelectedSeverity] = useState('');     // '1', '2', '3', '4', '5'
  const [selectedInfra, setSelectedInfra] = useState('');           // ID Kategori (1-7)
  const [selectedStatus, setSelectedStatus] = useState('');         // 'pending', 'proses', 'selesai', 'ditolak'

  // Mapping Kategori Infrastruktur Kelompok Laksana
  const infraMapping = [
  ...
  // --- LOGIKA AKSI HAPUS LAPORAN DI TABEL ---
  ...
  // 🌟 LOGIKA FILTER DATA SECARA REAL-TIME 
  ...
  // Helper styles badge kustom tingkat keparahan
  ...
  // Helper styles badge kustom status
    ...
    <div className="flex h-screen bg-[#f8fafc] font-sans overflow-hidden text-slate-700">
      ...
      <aside className="w-20 bg-white border-r border-slate-100 flex flex-col items-center py-6 space-y-8 flex-shrink-0">
        ...
        <nav className="flex flex-col space-y-6 text-slate-300">
          <button onClick={() => navigate('/dashboard')} className="text-teal-600 p-2 rounded-xl bg-teal-50/50"><i className="fa-solid fa-house text-lg"></i></button>
          
          {/* 🔗 LINK KE HALAMAN LAPORAN ANDA */}
          <button 
            onClick={() => navigate('/my-reports')} 
            className="hover:text-teal-600 p-2 transition-colors"
            title="Laporan Anda"
          >
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-slate-100 px-8 flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-xs font-medium text-slate-400">Nasional · <span className="text-teal-600 font-bold">{reports.length} laporan aktif</span></p>
          </div>
          
          <div className="flex items-center space-x-4">
            {/* TABS INTERAKTIF PETA VS TABEL */}
            <div className="flex bg-slate-100 p-1 rounded-xl text-xs font-bold">
              <button 
                onClick={() => setActiveView('peta')}
                className={`px-4 py-2 rounded-lg transition-all ${activeView === 'peta' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-400'}`}
              >
                🗺️ Peta
              </button>
              <button 
                onClick={() => setActiveView('tabel')}
                className={`px-4 py-2 rounded-lg transition-all ${activeView === 'tabel' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-400'}`}
              >
                📊 Tabel
              </button>
            </div>

            <button onClick={() => setIsModalOpen(true)} className="bg-teal-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold hover:bg-teal-700 shadow-md transition-all">+ Buat laporan</button>
            
            {/* 🔗 AVATAR PROFIL KLIK EDIT PROFIL */}
            <div 
        ...
        {/* 3. HORIZONTAL FILTERS COMPONENT */}
        <div className="bg-white border-b border-slate-100 p-6 space-y-4 flex-shrink-0 shadow-3xs">
          ...
          {/* FILTER 2: CATEGORY INFRASTRUKTUR */}
          <div className="flex items-center space-x-2 text-[10px] font-bold text-slate-400">
        ...
        {/* 4. WORKSPACE DISPLAY CONTENT */}
        <div className="flex-1 p-6 overflow-auto">
          
          {/* SCREENSHOT VIEW A: PETA INTERAKTIF */}
          {activeView === 'peta' && (
          ...
          {/* SCREENSHOT VIEW B: TABEL DATA DETAIL */}
          {activeView === 'tabel' && (
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
                    {filteredReports.length === 0 ? (
                      <tr>
                        <td colSpan={8} className="text-center py-10 text-slate-400 font-bold">Tidak ada laporan yang cocok dengan kriteria filter.</td>
                      </tr>
                    ) : (
                      filteredReports.map((report) => {
                          ...
                          <tr key={report.id} className="hover:bg-slate-50/40 transition-colors">
                            ...
                            <td className="py-4 px-4">
                              <div className="flex items-center justify-center space-x-3 text-slate-400">
                                <button className="hover:text-teal-600 transition-colors" title="Edit Laporan"><i className="fa-regular fa-pen-to-square text-sm"></i></button>
                                <button onClick={() => handleDelete(report.id)} className="hover:text-red-500 transition-colors" title="Hapus Laporan"><i className="fa-regular fa-trash-can text-sm"></i></button>
                              </div>
                            </td>
                          </tr>
                        );
      ...
      {/* --- RENDER SEMUA BOX MODAL POP-UP --- */}
      <FormLaporan isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onRefresh={fetchLaporan} />
